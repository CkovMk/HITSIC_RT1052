...
- {id: CCM.CSI_PODF.scale, value: '6', locked: true}
...
- {id: CCM.SPDIF0_CLK_SEL.sel, value: CCM_ANALOG.PLL3_PFD2_CLK}
...
- {id: CCM_ANALOG.PLL3_PFD0_DIV.scale, value: '27', locked: true}
...
/*******************************************************************************
 * Variables for sysclk_run configuration
 ******************************************************************************/
const clock_arm_pll_config_t armPllConfig_sysclk_run =
    {
        .loopDivider = 100,                       /* PLL loop divider, Fout = Fin * 50 */
        .src = 0,                                 /* Bypass clock source, 0 - OSC 24M, 1 - CLK1_P and CLK1_N */
    };
const clock_sys_pll_config_t sysPllConfig_sysclk_run =
    {
...
const clock_usb_pll_config_t usb1PllConfig_sysclk_run =
    {
        .loopDivider = 0,                         /* PLL loop divider, Fout = Fin * 20 */
        .src = 0,                                 /* Bypass clock source, 0 - OSC 24M, 1 - CLK1_P and CLK1_N */
    };
/*******************************************************************************
 * Code for sysclk_run configuration
 ******************************************************************************/
void RTECLK_Run600M(void)
{
    ...
    CLOCK_SetMux(kCLOCK_SpdifMux, 1);
    ...
    /* Init ARM PLL. */
    CLOCK_InitArmPll(&armPllConfig_sysclk_run);
    /* In SDK projects, SDRAM (configured by SEMC) will be initialized in either debug script or dcd.
     * With this macro SKIP_SYSCLK_INIT, system pll (selected to be SEMC source clock in SDK projects) will be left unchanged.
     * Note: If another clock source is selected for SEMC, user may want to avoid changing that clock as well.*/
#ifndef SKIP_SYSCLK_INIT
...
#endif
    /* Init System PLL. */
    CLOCK_InitSysPll(&sysPllConfig_sysclk_run);
    /* Init System pfd0. */
...
#if !(defined(XIP_EXTERNAL_FLASH) && (XIP_EXTERNAL_FLASH == 1))
    /* Init Usb1 PLL. */
    CLOCK_InitUsb1Pll(&usb1PllConfig_sysclk_run);
    /* Init Usb1 pfd0. */
    ...
    SystemCoreClock = SYSCLK_RUN_CORE_CLOCK;
}
